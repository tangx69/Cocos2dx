...
function SceneLauncher:getVersionCode()
	...
	if luaj then
		...
		local _ok,_ret  = luaj.callStaticMethod("com/xxxxx/utils/Utils", "getVersionName", args, sigs)
		if _ok then
			ret = _ret
		end
	elseif luaoc then
...
function SceneLauncher:getPackageName()
	...
	if luaj then
		local args = {}
		local sigs = "()Ljava/lang/String;"
		local _ok,_ret  = luaj.callStaticMethod("com/xxxxx/utils/Utils", "getPackageName", args, sigs)
		if _ok then
			ret = _ret
		end
	elseif luaoc then
...
function SceneLauncher:getMetaData(key)
	...
	if luaj then
		...
		local _ok,_ret  = luaj.callStaticMethod("com/xxxxx/utils/Utils", "getMetaData", args, sigs)
		if _ok then
            if _ret ~= "" then
                ret = _ret
            end

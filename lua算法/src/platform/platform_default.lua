...
function RegisterPage:_sendRegister(userNameStr, passwordStr,oldUser)
    ...
    if oldUser then
        urlStr = string.format("%s?username=%s&passwd=%s&oldusername=%s&oldpasswd=%s&mac=%s&spid=%s&device_type=%s&channelType=%d",url,userNameStr,passwordStr,oldUser.userName,oldUser.password,zzy.cUtils.getDeviceID(), zzy.config.ChannelID, zzy.StringUtils:urlencode(zzy.cUtils.getDeviceModel()), tonumber(channelType))
    else
        urlStr = string.format("%s?username=%s&passwd=%s&mac=%s&spid=%s&device_type=%s&channelType=%d",url,userNameStr,passwordStr,zzy.cUtils.getDeviceID(), zzy.config.ChannelID, zzy.StringUtils:urlencode(zzy.cUtils.getDeviceModel()), tonumber(channelType))
    end
...
function LoginPage:sendLogin(userNameStr, passwordStr,isGuest)
    ...
    local urlStr = string.format("%s?username=%s&passwd=%s&mac=%s&spid=%s&device_type=%s&channelType=%d",url,userNameStr,passwordStr,
            zzy.cUtils.getDeviceID(), zzy.config.ChannelID, zzy.StringUtils:urlencode(zzy.cUtils.getDeviceModel()), tonumber(channelType))
...
zzy.Sdk.init = function()
    ...
    local platFormInfo = {
        ...
        loginUrl = "http://115.159.66.225:17199/yjsdk/login",
        payUrl = "http://115.159.66.225:17199/yjsdk/pay"
    }
...
zzy.Sdk.openLogin = function()
    ...
    if user and user.userName and user.password then
        ...
        local userNameStr = user.userName
        local passwordStr = user.password
		local url = LOGIN_URL
        local channelType = zzy.cUtils.getCustomParam()
        local urlStr = string.format("%s?username=%s&passwd=%s&mac=%s&spid=%s&device_type=%s&channelType=%d",url,userNameStr,passwordStr,
                zzy.cUtils.getDeviceID(), zzy.config.ChannelID, zzy.StringUtils:urlencode(zzy.cUtils.getDeviceModel()), tonumber(channelType))
